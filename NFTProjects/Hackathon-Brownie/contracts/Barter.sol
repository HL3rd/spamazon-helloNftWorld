...
contract Barter is IERC721Receiver {
    ...
    // function stringer(address _contract, uint256 tokenID)
    //     public
    //     view
    //     returns (bytes memory)
    // {
    //     return abi.encodePacked(_contract, Strings.toString(tokenID));
    // }

    function number(uint256 tokenID) public view returns (bytes memory) {
        //return Strings.toString(tokenID);
        return abi.encodePacked(tokenID);
    }

    function stringer2(address _contract, uint256 tokenID)
        public
        view
        returns (bytes memory)
    {
        return abi.encodePacked(_contract, tokenID);
    }
